...
- Research on the web (market, competitors, docs, comparisons, landscape) → delegate to
  `deep-researcher`, which fetches real sources and correlates them.
...
- Never answer a research question from parametric memory and present it as current fact.
  If delegation is not feasible, answer with explicit `[UNVERIFIED]` markers.
- This applies to the whole operation: any internet research goes through the team first.
